...
「设计给 iPad」的 App Store 游戏（问道手游、阴阳师等）的触摸事件，由系统框架层从 session 级鼠标事件流翻译而来。绕过这一层的 `postToPid` 送进去的事件没人翻译，实测**完全无响应**（问道、阴阳师均已验证）。所以这类目标只能走 session 投递，并因此带来下面两条代价。

> **怎么判断自己装的是哪一种**：`Foo.app` 里如果是 `Wrapper/` + `WrappedBundle`，就是 App Store「设计给 iPad」版；如果 `Info.plist` 直接在根目录，就是 PlayCover 侧载版。**同一个 bundle id 两种都可能**，光看名字判断不了。PlayCover 侧载版把应用装成了标准 macOS 应用，窗口是普通 AppKit 窗口，行为未必与上面相同——本项目尚未验证，别直接套用这里的结论。
...
**代价二：后台点击会把游戏窗口切到前台。**
被点到的后台窗口会被系统激活，这一点无法从应用侧阻止。v1.7.2 把所有能想到的公开 API 路径都实测了一遍，全部失败：

| 尝试 | 结果 |
|---|---|
| `eventTargetUnixProcessID` 进程标记 | 点得中，**照样激活** |
| 填 `kCGMouseEventWindowUnderMousePointer` 等窗口字段 | 点得中，**照样激活** |
| 改投 `.cgAnnotatedSessionEventTap` | **点击直接失效**（换端口游戏就收不到了） |
| 上面两项叠加 | 点击失效 |

最后一行反过来说明：「游戏能收到点击」和「事件必须经过那条会激活窗口的路」是绑死的，不是没找对开关。

因此宏的默认策略是**仅在目标处于前台时执行**：目标不在前台就跳过这一步，你切回去它自动续跑。需要「一边干别的一边挂机」的话，可以把后台宏策略改成「允许后台执行」——代价是每跑一步游戏窗口都会翻到最前面，键盘焦点也会短暂被抢走。这个开关默认关闭。
...
**这一版围绕「后台宏到底能做到什么」做了一轮彻底的实测，修掉了三个真实故障，并把做不到的事写清楚。**

- **修复**：后台宏不再让前台在游戏和你的窗口之间来回横跳。原因是后台宏点第一下就把目标顶到前台，第二步再读「目标在不在前台」得到的是「在」，于是被当成用户自己切过去的，后台保护成片失效——防抖还原不再刷新，挂起中的那次照样触发，紧接着又被下一步点击抢回去。现在会单独记账「前台是宏自己抢来的、之前用户在哪」，保护不再失效
- **修复**：连击不再把鼠标箭头拽到游戏里回不来。连击原先在调用方 for 循环发多次，而「记下原位 → 移到点击点 → 还原」这套是按一次调用配平的；连击零间隔背靠背投递，第二发取原位时第一发的还原还没落定，取到的是已经被移过去的位置，收尾就把光标「还原」到了点击点上。现在连击合并进一次投递序列，原位只取一次
- **修复**：后台宏不再偶尔丢步骤。iOS-on-Mac 应用在后台被系统限流（降帧、暂停输入处理），而原先点击和窗口激活几乎同时发生，游戏在刚被唤醒那几帧接不住。现在改为先主动把目标唤到前台、等 150ms 缓过来再投递
- **改进**：「仅前台执行」不再约束原生 macOS 应用。这一档的代价只存在于 iOS-on-Mac 目标；原生应用走 `postToPid`，后台点击不动光标、不激活窗口、也不受遮挡影响，跳过它纯属白白牺牲功能。遮挡安全网同理，现在只对 session 投递生效
- **改进**：你自己切到目标窗口时，挂起中的「还原前台」会立刻放弃。后台宏的触发键只在目标处于前台时才拦得到，也就是说你为了停宏必须先切回游戏——原先会在 150ms 后把你弹回原来的窗口，看起来就是「点了游戏又被踢出来」
- **改进**：被遮挡而跳过时的日志改为节流 + 说清原因和解法。原先每步刷一行，几百轮的宏会把日志淹掉，而且只说「被遮挡」不说该怎么办
- **文档**：修正两处会误导排查的注释与说明——`postToPid` 无效的结论只在 App Store「设计给 iPad」版上验证过，不能推广到 PlayCover 侧载版；以及**同一个 bundle id 两种安装形态都可能**，要看 bundle 布局而不是 id
