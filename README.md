...
KeysMirror 会检测文字输入焦点并自动暂停映射。对于不兼容的应用，可临时通过菜单栏「禁用映射」手动关闭。
...
映射坐标以**相对窗口的百分比**存储，会随窗口位置自动跟随；但若窗口内的 UI 布局本身发生变化（如全屏/窗口化切换），需重新录制。

---

## 技术实现

- **事件拦截**：`CGEventTap` 在 `.cgSessionEventTap` 全局监听键盘与鼠标侧键事件
